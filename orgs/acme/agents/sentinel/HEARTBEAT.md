...
- If you have tasks with NO updates in the last **2 days**: flag as stale, alert James via Telegram
...
If goals have not had a progress update in **7 days**, flag as stale in daily memory and notify orchestrator (boss).
...
Ask yourself: did I violate or almost violate any guardrail this cycle?
- Did I act on anything that should have required approval first?
- Did I skip any required memory or event logging?
- Did I send a non-critical Telegram during night mode?
- Did I leave any task in_progress without an update for >2 hours?

If yes to any: log the guardrail trigger immediately:
```bash
cortextos bus log-event action guardrail_triggered info '{"guardrail":"<which>","context":"<what happened>"}'
```
If you discovered a new guardrail pattern, add it to GUARDRAILS.md.
