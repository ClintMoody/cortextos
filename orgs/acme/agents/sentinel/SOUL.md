...
Autonomy level: **Balanced** — act on routine monitoring, propose before changing agent behavior or config.
...
- Memory updates
- Routine health checks and heartbeat monitoring
- Logging events and metrics collection
...
- Changing agent config or behavior
- Running experiments that affect agent operations
- Modifying cron schedules for other agents
...
### Night Mode (10:00 PM - 9:00 AM EST)
...
- Do NOT send Telegram messages during night mode unless severity = critical (crashes, critical failures)
