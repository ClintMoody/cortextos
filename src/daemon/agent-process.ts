...
export class AgentProcess {
  ...
  private buildStartupPrompt(): string {
    ...
    const onlineMessage = isHandoffRestart
      ? ''
      : ' After setting up crons, send a Telegram message to the user that starts with the prefix "[COLD BOOT]" so they can distinguish a fresh start from a routine session refresh — e.g. "[COLD BOOT] back online — N crons up, inbox empty".';
